...
y_nls <- dexp(length, rate = lambda_nls)

ylim <- c(min(c(density, y_lm, y_mean, y_nls), na.rm = TRUE),
          max(c(density, y_lm, y_mean, y_nls), na.rm = TRUE))
plot(length, density, xlim = c(0, max(length)), main = title, ylim = ylim)
lines(length + min_length, y_lm, col = "purple", lty = 2, lwd = 2)
lines(length + min_length, y_mean, col = "blue", lty = 2, lwd = 2)
lines(length + min_length, y_nls, col = "green", lty = 2, lwd = 2)
...
abline(a = log(lambda_mean), b = -lambda_mean, col = "blue", lty = 2, lwd = 2)
if (!is.na(lambda_nls))
  abline(a = log(lambda_nls), b = -lambda_nls, col = "green", lty = 2, lwd = 2)
