...
pdf("admixture_dating.pdf", width = 14, height = 8)
...
title <- sprintf("sample age %d, min length %d bp, max length %d Mb", sample_age, min_length, max_length / 1e6)

legends <-c(sprintf("lm fit of t = %.1f kya, lambda = %f", round(t_lm / 1e3, 1), lambda_lm),
            sprintf("MLE fit of t = %.1f kya, lambda = %f", round(t_mean / 1e3, 1), lambda_mean))

# plot the results on the original exponential scale
y_lm <- dexp(length, rate = lambda_lm)
y_mean <- dexp(length, rate = lambda_mean)

plot(length, density, xlim = c(min_length, max(length)), main = title)
...
plot(length, log(density), xlim = c(min_length, max(length)), main = title)
...
p_tracts_full <-
  ggplot() +
  geom_histogram(data = tracts, aes(x = length, y = after_stat(density), fill = as.factor(time)),
...
```{r}
exp_decay <- function(lambda, min_length, max_length) {
  data.frame(length = seq(min_length, max_length, by = 100000)) %>%
...
predictions_df <-
  exp_df %>%
  rowwise() %>%
  mutate(exp_data = list(exp_decay(lambda, min(tracts$length), max(tracts$length)))) %>%
  unnest(cols = c(exp_data))

p_fit_full <- p_tracts_full +
  geom_line(data = predictions_df, aes(x = length, y = density),
...
## Distribution of tract lengths > 50 kb

```{r}
tracts <- tracts_df %>% filter(length > 50e3)
```
...
ggplot() +
  ...
  geom_density(data = filter(tracts_df, length >= 50e3),
               aes(length, y = after_stat(count), color = factor(time)),
