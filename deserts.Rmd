...
fit_trunc_nls2 <- nls(counts ~ SSasymp(bins, Asym, R0, lrc), weights = 1 / bins)
...
fit_trunc_nls2 <- nls(props ~ SSasymp(bins, Asym, R0, lrc), na.action = na.omit)
summary(fit_trunc_nls2)
lines(bins[!is.na(counts)], predict(fit_trunc_nls2), col = "green")
...
# https://www.science.org/doi/10.1126/sciadv.abm7047#:~:text=Within%20this%20time%20frame%2C%20we,of%2023.2%20±%203.0%20years
```
...
```{r}
samples <- metadata %>%
  mutate(t_admix = lapply(sampleId, function(x) {
bin_step <- 25e3

tract_bins <- tracts_df %>% filter(sampleId == x)
...
list(t_admix = t_est, t_admix_lower = t_lower, t_admix_upper = t_upper)
  })) %>% tidyr::unnest_wider(t_admix)
```

```{r}
samples %>%
ggplot() +
  geom_boxplot(aes(groupAge, t_admix)) +
  geom_jitter(aes(groupAge, t_admix))
```

```{r}
ggplot(samples) +
  geom_pointrange(aes(sampleId, t_admix, ymin = t_admix_lower, ymax = t_admix_upper)) +
  facet_wrap(~ groupAge, scales = "free_x")
```

```{r}
samples %>% ggplot() + geom_histogram(aes(ageAverage))
```
