...
for (min_length in c(0, 25e3, 50e3, 75e3, 100e3)) {
...
for (min_length in c(0, 50e3, 100e3, 150e3, 200e3, 250e3, 300e3)) {
  ...
  results_v2[[length(results_v2) + 1]] <- fit_df %>% mutate(min_length = min_length)
}
...
```{r}
inner_join(
  filter(results_df, method == "nls") %>% select(sample_age, min_length, t_inferred),
...
for (i in seq_along(all_inds)) {
  ...
  bin_data <- hist(ind_tracts_df$length, plot = FALSE, breaks = 100)
  ...
  failed_nls <- !inherits(nls_res, "nls")
  lambda_nls <- if (failed_nls) NA else exp(unname(coef(nls_res)["lrc"]))
  t_gens_before <- lambda_nls / r
  t_inferred <- t_gens_before * gen_time + sample_age
  
  x_values <- sort(unique(c(length, seq(0, max(ind_tracts_df$length), by = 5000))))
  y_nls <- if (failed_nls) NA else predict(nls_res, newdata = data.frame(length = x_values + min_length))
  predictions_df <- tibble(length = x_values, density = y_nls)

  ind_df <- tibble(name = ind, sample_age = sample_age, t_inferred = t_inferred, min_length = min_length)
  ...
  results_v3[[length(results_v3) + 1]] <- ind_df
}

dev.off()

results_v3_df <- do.call(rbind, results_v3)
```

```{r}
x_df <- inner_join(results_v3_df, metadata, by = c("name" = "sampleId", "sample_age" = "ageAverage")) %>%
  mutate(cluster = case_when(t_inferred < 48000 ~ "x", t_inferred > 52000 ~ "y", TRUE ~ "z"))

{ x_df %>%
  ggplot() +
  geom_point(aes(sample_age, t_inferred, color = cluster)) +
...
```{r}
pdf("dating_tracts_inds.pdf")
for (ind in tracts_df %>% arrange(-ageAverage) %>% pull(sampleId) %>% unique) {
  {
    filter(tracts_df, sampleId == ind) %>%
    ggplot() +
    geom_density(aes(length)) +
    coord_cartesian(xlim = c(0, 2e6), ylim = c(0, 2e-5)) +
    ggtitle(paste0(ind, ", ", filter(metadata, sampleId == ind)$ageAverage, " years old"))
  } %>%
    print()
}
dev.off()
...
all_inds <- tracts_df %>% arrange(-ageAverage) %>% pull(sampleId) %>% unique
...
for (i in seq_along(all_inds)) {
  ...
  ind_tracts_df <- filter(tracts_df, length >= min_length, sampleId == ind)
  sample_age <- ind_tracts_df$ageAverage[1]
