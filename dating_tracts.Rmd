...
results_ind_breaks <- list()

all_inds <- tracts_df %>% arrange(-sample_age) %>% pull(name) %>% unique

# for (breaks in c(10, 25, 100, 250, 500, 1000)) {
for (breaks in c(101)) {

results_ind <- list()

pdf(paste0("dating_tracts_empirical_inds_breaks", breaks, ".pdf"), width = 12, height = 8)

for (i in seq_along(all_inds)) {
  ind <- all_inds[i]
  cat(sprintf("Processing individual %s [%d/%d] (breaks %d)\n", ind, i, length(all_inds), breaks))

  ind_tracts_df <- filter(tracts_df, length >= min_length, name == ind)
  sample_age <- unique(ind_tracts_df$sample_age)
  age_group <- unique(ind_tracts_df$age_group)

  bin_data <- hist(ind_tracts_df$length, plot = FALSE, breaks = breaks)
  density <- bin_data$density
  ...
  results_ind[[length(results_ind) + 1]] <- ind_df %>% mutate(breaks = breaks)
}

dev.off()

results_ind_breaks[[breaks]] <- do.call(rbind, results_ind)
}

results_ind_df <- do.call(rbind, results_ind_breaks)
```
