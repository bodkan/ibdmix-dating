...
unlink("dating_tracts.*.pdf", recursive = TRUE)
```
...
### Individual MLE estimates

```{r}
tracts_df <- tracts_df %>% mutate(chrom = "chr1") %>%
  split(.$name) %>%
  lapply(function(df) {
    gr <- makeGRangesFromDataFrame(df, start.field = "left", end.field = "right", keep.extra.columns = TRUE) %>% reduce()
    as.data.frame(gr) %>% select(chrom = seqnames, left = start, right = end, length = width) %>%
      mutate(name = df$name[1], sample_age = df$sample_age[1])
  }) %>%
  do.call(rbind, .)
```
...
results_ind_mle <- list()

max_length <- Inf

all_inds <- tracts_df %>% arrange(-sample_age) %>% pull(name) %>% unique

pdf("dating_tracts_sim_inds_v1_mle.pdf", width = 12, height = 8)
...
for (i in seq_along(all_inds)) {
  ...
  exp_df <- group_by(ind_tracts_df, name) %>%
    summarise(L = mean(length)) %>%
    mutate(
      sample_age = sample_age,
      lambda = 1 / (L - min_length),
  ...
  predictions_df <-
    ...
    select(name, lambda) %>%
    rowwise() %>%
    mutate(exp_data = list(exp_decay(lambda, max(ind_tracts_df$length)))) %>%
    unnest(cols = c(exp_data)) %>%
    select(-lambda)
  ...
  results_ind_mle[[length(results_ind_mle) + 1]] <- exp_df %>% mutate(min_length = min_length, fit = list(select(predictions_df, -name)))
}

dev.off()

results_ind_mle_df <- do.call(rbind, results_ind_mle)
```
...
```{r}
results_ind_mle_df %>%
ggplot(aes(sample_age, t_gen, color = factor(sample_age))) +
  geom_jitter(alpha = 0.75) +
  geom_smooth(aes(group = 1)) +
  coord_cartesian(ylim = c(0, 3000)) +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  labs(x = "sample age group", y = "time of admixture since the sample age",
       title = "Inferred admixture time in different sample groups",
...
ind_tracts_df <- tracts_df %>% filter(name == !!name, length >= min_length) 
ind_df <- filter(results_ind_mle_df, name == !!name)

p_ind_tracts <- ind_tracts_df %>%
  ggplot() +
...
p_ind_fit <- ggplot() +
  geom_histogram(
    data = ind_tracts_df, aes(x = length, y = after_stat(density)),
    binwidth = 20000, alpha = 0.75
  ) +
  geom_line(data = ind_df$fit[[1]], aes(length + min_length, density)) +
...
### Individual NLS estimates

```{r}
min_length <- 50e3

results_ind_nls <- list()

max_length <- Inf

all_inds <- tracts_df %>% arrange(-sample_age) %>% pull(name) %>% unique

pdf("dating_tracts_sim_inds_v2_nls.pdf", width = 12, height = 8)
...
for (i in seq_along(all_inds)) {
  ...
  ind_df <- tibble(
    name = ind, sample_age = sample_age, t_inferred = t_inferred, min_length = min_length, t_before = t_gens_before,
  ...
  results_ind_nls[[length(results_ind_nls) + 1]] <- ind_df
}

dev.off()

results_ind_nls_df <- do.call(rbind, results_ind_nls)
```
...
```{r}
results_ind_nls_df %>%
ggplot(aes(sample_age, t_before, color = factor(sample_age))) +
  geom_jitter(alpha = 0.75) +
  coord_cartesian(ylim = c(0, 3000)) +
  geom_smooth(aes(group = 1)) +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
...
ind_df <- filter(results_ind_nls_df, name == !!name)
...
pdf("dating_tracts_empirical_inds_mle.pdf", width = 12, height = 8)
...
for (i in seq_along(all_inds)) {
  ...
  results_ind_mle[[length(results_ind_mle) + 1]] <- exp_df %>% mutate(min_length = min_length, fit = list(predictions_df))
}
...
```{r}
saveRDS(results_ind_mle_df, "dating_tracts_empirical_inds_mle.rds")
```

```{r}
results_ind_mle_df <- readRDS("dating_tracts_empirical_inds_mle.rds")
```
...
```{r}
results_ind_mle_df %>%
ggplot(aes(age_group, t_gen, color = age_group, group = name)) +
  geom_jitter(alpha = 0.75) +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  labs(x = "sample age group", y = "inferred admixture time",
       title = "Inferred admixture time in different sample groups",
...
```{r}
results_ind_mle_df %>%
ggplot(aes(sample_age, t_gen, color = age_group)) +
  geom_jitter(alpha = 0.75) +
  geom_smooth(aes(group = 1)) +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  labs(x = "sample age group", y = "inferred admixture time",
...
ind_tracts_df <- tracts_df %>% filter(name == !!name) 
ind_df <- filter(results_ind_mle_df, name == !!name)
...
pdf("dating_tracts_empirical_inds_nls.pdf", width = 12, height = 8)
...
for (i in seq_along(all_inds)) {
  ...
  not_empty <- density > 0
  length <- length[not_empty]
  density <- density[not_empty]
  data_df <- tibble(length, density)

  p_tracts <-
    ggplot() +
    geom_bar(data = data_df, aes(x = length, y = density), alpha = 0.75, stat = "identity") +
    labs(
      x = "tract length [bp]", y = "density", fill = "age of sample",
      title = "Distribution of introgressed tracts",
      subtitle = paste0("name: ", ind, ", date: ", sample_age," years BP")
...
```{r}
saveRDS(results_ind_nls_df, "dating_tracts_empirical_inds_nls.rds")
```

```{r}
results_ind_nls_df <- readRDS("dating_tracts_empirical_inds_nls.rds")
```
