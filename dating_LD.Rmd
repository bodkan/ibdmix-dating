...
sites_grl <- GRangesList(list(
  makeGRangesFromDataFrame(mutate(info_gt, chrom = "chr1_1"), start.field = "pos", end.field = "pos"),
  makeGRangesFromDataFrame(mutate(info_gt, chrom = "chr1_2"), start.field = "pos", end.field = "pos")
))
seqlevels(sites_grl) <- seqlevels(tracts_gr)
seqlengths(sites_grl) <- seqlengths(tracts_gr)
...
# unlink("dating_LD_pairs_regular.rds")
if (file.exists("dating_LD_pairs_mutated.rds")) {
  pairs <- readRDS("dating_LD_pairs_mutated.rds")
} else {
  tstart <- Sys.time()
  pairs <- collect_pairs(sites_grl, distances)
  tend <- Sys.time()
  ...
  print(tend - tstart)
  saveRDS(pairs, "dating_LD_pairs_mutated.rds")
}
```
