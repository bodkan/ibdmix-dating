...
sites_gr <- sites_grl[seqlevels(sites_grl) %in% chrom, ] %>% unlist
ind_tracts_gr <- tracts_gr %>% filter(name == !!name, seqnames %in% chrom)
ind_sites_gr <- sites_gr

# mark sites falling within an introgressed tract
tract_overlaps <- queryHits(findOverlaps(ind_sites_gr, ind_tracts_gr))
mcols(ind_sites_gr)$tract <- FALSE
mcols(ind_sites_gr[tract_overlaps])$tract <- TRUE
mcols(ind_sites_gr)$tract <- as.integer(mcols(ind_sites_gr)$tract)

# data.table is stupid:
ch <- chrom
matches_dt <-
  info_gt[chrom == ch,
          .(hap1_match = as.integer(get(paste0(name, "_hap1")) == NEA_1),
...
p <- ggplot() +
  geom_rect(data = as.data.frame(ind_tracts_gr), aes(xmin = start, xmax = end, ymin = 0.1, ymax = 1.5), fill = "blue") +
  geom_point(data = as.data.frame(filter(ind_sites_gr, tract == 1)), aes(x = start, y = 0.05), size = 0.1, color = "darkgreen") +
  geom_point(data = as.data.frame(filter(ind_sites_gr, hap1_match == 1)), aes(x = start, y = 0.03), size = 0.1, color = "orange") +
  geom_point(data = as.data.frame(filter(ind_sites_gr, hap2_match == 1)), aes(x = start, y = 0.01), size = 0.1, color = "pink") +
  geom_point(data = as.data.frame(ind_sites_gr), aes(x = start, y = 0), size = 0.1, alpha = 0.05) +
  scale_x_continuous(labels = scales::comma) +
  expand_limits(x = 0) +
  theme_minimal() +
