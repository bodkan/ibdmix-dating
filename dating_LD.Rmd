...
if (file.exists("dating_LD_pairs_regular.rds")) {
  pairs <- readRDS("dating_LD_pairs_regular.rds")
} else {
  tstart <- Sys.time()
...
saveRDS(cov_df, "dating_LD_regular_cov.rds")
```

```{r}
cov_reg_df <- readRDS("dating_LD_regular_cov.rds")
```
...
info_sites <- info_gt %>% mutate(index = 1:sum(fixed_sites)) %>% select(pos, index)
```

```{r}
sites_grl <- GRangesList(list(
  makeGRangesFromDataFrame(mutate(info_sites, chrom = "chr1_1"), start.field = "pos", end.field = "pos", keep.extra.columns = TRUE),
  makeGRangesFromDataFrame(mutate(info_sites, chrom = "chr1_2"), start.field = "pos", end.field = "pos", keep.extra.columns = TRUE)
))
...
saveRDS(cov_df, "dating_LD_mutations_cov.rds")
```

```{r}
cov_mut_df <- readRDS("dating_LD_mutations_cov.rds")
```
